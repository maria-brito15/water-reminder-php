...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['excluir_conta'])) {
    try {
        ...
        if (!empty($novoNome) && $novoNome !== $usuario->getNome()) {
        ...
        if (isset($_POST['intervalo'])) {
            $novoIntervaloHoras   = floatval($_POST['intervalo']);
            $novoIntervaloSegundos = (int)($novoIntervaloHoras * 3600);

            if ($novoIntervaloSegundos !== $usuario->getIntervalo()) {
                if ($db->atualizarUsuarioIntervalo($_SESSION['user_id'], $novoIntervaloSegundos)) {
